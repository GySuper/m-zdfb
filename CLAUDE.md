...
本项目所有编码、重构、Code Review 工作,**Claude Code 必须严格遵循以下 4 条守则**。它们是顶层行为约束,优先级高于本文件其他章节的技术细节(冲突时以这 4 条为准)。
...
> ⚠️ 这 4 条与本文件其他具体建议冲突时,以这 4 条为准;若仍不确定,停下来问用户。
...
5. 不一次性写完,遵循守则 #4(目标驱动执行):每步先定可验证标准,跑通再继续

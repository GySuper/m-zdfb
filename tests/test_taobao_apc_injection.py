"""淘宝光合 publisher.publish() 的 APC 故障注入(镜像 test_publisher_apc_injection.py)。

`check_pass()=False` 时,_pre_publish 必须在 step [login] 之后:
...
风格对齐 tests/test_taobao_publisher.py:基础设施 patch 打 runner,步骤 patch 打 taobao_guanghe。
"""
...
def test_taobao_apc_pass_runs_full_pipeline(
...
def test_taobao_apc_deny_injects_element_not_found(
    pending_task: tuple[int, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_taobao_dev_mode_no_apc_call(
    pending_task: tuple[int, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
